...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (product) => {
    dispatch(addItem(product)); // Dispatch the addItem action to add the product to the cart (Redux action)
  ...
  const styleObjUl = {
    // Inline styles for the navbar links
    display: "flex",
    justifyContent: "space-between",
    alignItems: "center",
    width: "1100px",
  ...
  const styleA = {
    // Inline styles for the anchor tags in the navbar
    color: "white",
    fontSize: "30px",
    textDecoration: "none",
  };
